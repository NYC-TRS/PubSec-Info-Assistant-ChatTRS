...
latest part of the conversation, avoiding common words (e.g., 'the', 'and'). Do not include filenames (e.g., .txt, .pdf), any text inside [ ] or <<<>>> 
...
async def run(self, history: Sequence[dict[str, str]], overrides: dict[str, Any], citation_lookup: dict[str, Any], thought_chain: dict[str, Any]) -> Any:
    ...
    if detectedlanguage != self.target_translation_language:
        user_question = self.translate_response(
            user_q, self.target_translation_language)
    else:
    ...
    vector = RawVectorQuery(vector=embedded_query_vector,
                            k=top, fields="contentVector")
    ...
    if (self.use_semantic_reranker and overrides.get("semantic_ranker")):
        ...
        r = self.search_client.search(
            generated_query, top=top, vector_queries=[
                vector], filter=search_filter
        )
...
def detect_language(self, text: str) -> str:
    ...
    try:
        endpoint_region = self.enrichment_endpoint.split(
            "https://")[1].split(".api")[0]
        ...
        response = requests.post(
            api_detect_endpoint, headers=headers, json=data)
        ...
        if response.status_code == 200:
            ...
            raise Exception(
                f"Error detecting language: {response.status_code}")
    except Exception as e:
        raise Exception(
            f"An error occurred during language detection: {str(e)}") from e
...
def translate_response(self, response: str, target_language: str) -> str:
    ...
    response = requests.post(api_translate_endpoint,
